...
*/	
	...
	//Dir = "C:\\MRM\\Metrolina\\2055"
	Dir = Args.[Run Directory]
	//sedata_file = Dir + "\\LandUse\\SE_2055.bin"
...
SetRandomSeed(42837)
  ...
  for tc = 1 to telecommute_ar.length do 	//loop on telecommuting scenarios 
  UpdateProgressBar("Telecommuting Scenario " + i2s(tc), 10) 
  
  	//Create files and move over tables
	DirTCscen = Dir + "\\TC_Scenario"
	//Open and modify HBW, XIN, & ATW files

// start with HBW
	hbw_file = OpenTable("hbw_file", "FFB", {Dir + "\\TD\\dcHBW.bin",})
...
//	SetDataVector(hbw_file+"|", "randnumOG", randnumOG,{{"Sort Order", {{"ID","Ascending"}}}})	
//	SetDataVector(hbw_file+"|", "randnumC", randnumC,{{"Sort Order", {{"ID","Ascending"}}}})	

	// Join HBW file to SE file to determine if it is a OffGov job
	jointab = JoinViews("jointab", "hbw_file.DEST_TAZ", "SEFile.TAZ", ) //{{"O",}})
	...
	CopyTableFiles("hbw_file", null, null, null, Dir + "\\TD\\dcHBW_preCommute.bin", null)
	...
	temp_file = OpenTable("temp_file", "FFB", {Dir + "\\TD\\dcHBW_preCommute.bin",})
	...
	ExportView("temp_file|nonTCset", "FFB", Dir + "\\TD\\dcHBW.bin",,)
	HBOaddset = SelectByQuery("HBOaddset", "Several", qry2)
	ExportView("temp_file|HBOaddset", "FFB", Dir + "\\TD\\dcHBO_TCadd.bin",,)
	CloseView("temp_file")
	...
	dcHBO_TCadd_file = OpenTable("dcHBO_TCadd_file", "FFB", {Dir + "\\TD\\dcHBO_TCadd.bin",})
	opts = {}
	...
	dcHBO = OpenTable("dcHBO", "FFB", {Dir + "\\TD\\dchbo.bin",})
...
skipahead:	
// now do XIW
	xiw_file = OpenTable("xiw_file", "FFB", {Dir + "\\TD\\dcXIW.bin",})
	...
	jointab = JoinViews("jointab", "xiw_file.DEST_TAZ", "SEFile.TAZ", )  // {{"O",}})
	OGpct = GetDataVector(jointab+"|", "OFFGOVPCT",{{"Sort Order", {{"ID","Ascending"}}}})
	OG = if randnumOG >= OGpct then 1 else 0
	telecommute = if (OG = 1 and randnumTC <= telecommute_ar[tc]) then 1 else 0
	CloseView("jointab")
	...
	// Make a copy of the file, holding on to all the original results
	CopyTableFiles("xiw_file", null, null, null, Dir + "\\TD\\dcXIW_preCommute.bin", null)

	CloseView("xiw_file")
	
	//Open new table, select if a telecommute tour, then remove from revised dcXIW table
	temp_file = OpenTable("temp_file", "FFB", {Dir + "\\TD\\dcXIW_preCommute.bin",})
	qry = "Select * where telecommute <> 1"
	SetView("temp_file")
	nonTCset = SelectByQuery("nonTCset", "Several", qry)
	ExportView("temp_file|nonTCset", "FFB", Dir + "\\TD\\dcXIW.bin",,)
	CloseView("temp_file")

// now do ATW, start with ii
	atw_file = OpenTable("atw_file", "FFB", {Dir + "\\TD\\dcATW.bin",})
	hbw_file = OpenTable("hbw_file", "FFB", {Dir + "\\TD\\dcHBW_preCommute.bin",})
	...
	// Make a copy of the file, holding on to all the original results
	CopyTableFiles("atw_file", null, null, null, Dir + "\\TD\\dcATW_preCommute.bin", null)
	CloseView("atw_file")	
	...
	atw_file = OpenTable("atw_file", "FFB", {Dir + "\\TD\\dcATW_preCommute.bin",})
	jointab = JoinViews("jointab", "atw_file.HBWID", "hbw_file.ID", ) // {{"O",}})	//do I need "O" here?
	...
	SetView("atw_file")
	nonTCset = SelectByQuery("nonTCset", "Several", qry)
	ExportView("atw_file|nonTCset", "FFB", Dir + "\\TD\\dcATW.bin",,)
	CloseView("atw_file")	
 ...
 //now do ATW_EXT: same as above, but also need to remove the ATWs from the dcEXT table, then re-merge with the new non-telecommuting records
	atwext_file = OpenTable("atwext_file", "FFB", {Dir + "\\TD\\dcATWext.bin",})
	...
	// Make a copy of the file, holding on to all the original results
	CopyTableFiles("atwext_file", null, null, null, Dir + "\\TD\\dcATWext_preCommute.bin", null)
	CloseView("atwext_file")	

	//Now open the new file, then join with HBW file to denote telecommuting tours, select only those that are not telecommute, and re-save as the new dcATW
	atwext_file = OpenTable("atwext_file", "FFB", {Dir + "\\TD\\dcATWext_preCommute.bin",})
	jointab = JoinViews("jointab", "atwext_file.HBWID", "hbw_file.ID", )//  {{"O",}})	//do I need "O" here?
	TC_v = GetDataVector(jointab+"|", "telecommute", ) // {{"Sort Order", {{"ID","Ascending"}}}})	
	CloseView("jointab")
	CloseView("hbw_file")
	...
	//Open the dcEXT file and save only the ATW records.  Also, create a temp table without any ATW records.
 	ext_file = OpenTable("ext_file", "FFB", {Dir + "\\TD\\dcEXT.bin",})
	...
	ExportView("ext_file|ATWset", "FFB", Dir + "\\TD\\dcATWonly.bin",,)	//dcEXT with only ATWs
//	ExportView("ext_file|ATWset", "FFB", Dir + "\\TD\\dcATWonly.bin",,{"Additional Fields", {{"NEWID", "Integer", 10, null, "No"},{"HBW_TC", "Integer", 1, null, "No"}}})	//dcEXT with only ATWs
	ExportView("ext_file|nonATWset", "FFB", Dir + "\\TD\\dcEXT_temp.bin",,)	//dcEXT with original ATW removed
//	ExportView("atw_file|nonTCset", "FFB", Dir + "\\TD\\dcATW.bin",,)
	CloseView("ext_file")	

	//FIll in NEWID (the original ID of the ATWext table), then join the pre-commute file to the only ATW dcEXT file
 	dcATWonly = OpenTable("dcATWonly", "FFB", {Dir + "\\TD\\dcATWonly.bin",})
	...
	jointab = JoinViews("jointab", "dcATWonly.NEWID", "atwext_file.ID", )//  {{"O",}})	//do I need "O" here?
	...
	ExportView("dcATWonly|TCset", "FFB", Dir + "\\TD\\dcATWextNONTC.bin",,)	//this is the temp ATWext file 
	CloseView("dcATWonly")	
	...
	//merge the stops table from ATWext telecommute tours to the dcEXT file
 	ext_file = OpenTable("ext_file", "FFB", {Dir + "\\TD\\dcEXT_temp.bin",})
 	newatwext_file = OpenTable("newatwext_file", "FFB", {Dir + "\\TD\\dcATWextNONTC.bin",})
	
	fields_ar = GetFields("ext_file", null)
/*	num = (fields_ar[1].length - 1) //need to remove the HBWID field since it's not in dcEXT
	dim fieldnames[num] 
	for j = 1 to num do
		if fields_ar[1][j] = "HBWID" then 
			goto gothbwidpos
	end
	gothbwidpos:
	for i = 1 to (j-1) do
...
*/	num = (fields_ar[1].length) 
	...
	ExportView("ext_file|", "FFB", Dir + "\\TD\\dcEXT.bin",,)	
	CloseView("ext_file")	
	CloseView("newatwext_file")	
		...
		// HOT
/*		DirHOT = Dir + "\\HwyAssn\\HOT\\"
		for i = 1 to hotassn_dcb.length do	
			WriteLine(batchhandle, "copy " + DirHOT + hotassn_dcb[i] + " " + DirTCscen + "\\HwyAssn\\HOT\\" + hotassn_dcb[i])
				else do
