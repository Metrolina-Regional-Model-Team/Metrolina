Macro "OPBusW_Assign" (Args)
	...
	AppendToLogFile(1, "Enter OPBusW_Assign: " + datentime)
...
// ----------------------------------- STEP 3: Transit Assignment Path Finder  -----------------------------------
     ...
     Opts.Input.[OD Matrix Currency] = {Dir + "\\tranassn\\Transit Assign Walk.mtx", "OPbusW", "Rows", "Columns"}
     Opts.Output.[Flow Table] = Dir + "\\tranassn\\OPbusW\\TASN_FLW.bin"
     Opts.Output.[Walk Flow Table] = Dir + "\\tranassn\\OPbusW\\TASN_WFL.bin"
     Opts.Output.[OnOff Table] = Dir + "\\tranassn\\OPbusW\\TASN_ONO.bin"
...
//     ret_value = RunMacro("TCB Run Procedure", 1, "Transit Assignment SP", Opts)
	...
	badbuildtrannet:
	msg = msg + {"OPBusW_Assign - Error return build transit network"}
	AppendToLogFile(1, "OPBusW_Assign - Error return build transit network") 
	goto badquit

	badtransettings:
	msg = msg + {"OPBusW_Assign - Error return from transit network settings"}
	AppendToLogFile(1, "OPBusW_Assign - Error return from transit network settings") 
	goto badquit

	badtranassn:
	msg = msg + {"OPBusW_Assign - Error return from transit network skims"}
	AppendToLogFile(1, "OPBusW_Assign - Error return from transit network skims")
	goto badquit

	badxpr_stopflags:
	msg = msg + {rtnmsg}
	AppendToLogFile(2, rtnmsg)
	msg = msg + {"OPBusW_Assign - Error return from XPR_StopFlags"}
	AppendToLogFile(1, "OPBusW_Assign - Error return from XPR_StopFlags")
	goto badquit
	...
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit OPBusW_Assign: " + datentime)
	AppendToLogFile(1, " ")

	return({OPBusWAssnOK, msg})
