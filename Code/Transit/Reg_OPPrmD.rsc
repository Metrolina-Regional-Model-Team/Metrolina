...
// 5/30/19, mk: There are now three distinct networks, use offpeak since was used to setup route system
	...
	AppendToLogFile(1, "Enter Reg_OPPrmD: " + datentime)
...
////
	Opts = RunMacro("create_tnet", "offpeak", "premium", "drive", Dir)
...
// ----------------------------------- STEP 2: Transit Network Setting  -----------------------------------
	...
	Opts = RunMacro("set_tnet", "offpeak", "premium", "drive", Dir)
...
//     Opts.Output.[OP Matrix].Label = "Prm OP Time Matrix (Off-Peak)"
//     Opts.Output.[OP Matrix].[File Name] = Dir + "\\skims\\TR_OP_SKIM_opprmd.mtx"
...
//	om = OpenMatrix(Dir + "\\skims\\TR_PARK_OPPRMD.mtx", "True")
//	CreateTableFromMatrix(om, Dir + "\\tranassn\\OPprmd\\pnrnode_opprmd.asc", "FFA", {{"Complete", "Yes"}})
	...
	OM = OpenMatrix(Dir + "\\skims\\TR_SKIM_OPPrmD.mtx", "True")
 ...
 //--- include the highway skim matrix as a part of Transit Skim Matrices . Added By Balaji
	...
	M1 = OpenMatrix(Dir+"\\skims\\TR_SKIM_OPPrmD.mtx", "True")
...
///////////////////////////////////
    ...
    M1 = null
     ...
     Opts = {{"Input",    {{"Matrix Currency",   {Dir + "\\skims\\TR_SKIM_OPPrmD.mtx",
                                                  "InVehGT5",
                                                  "RCIndex",
                                                  "RCIndex"}}}},
             {"Global",   {{"Method",            11},
...
// Add Premium in-vehicle time
    ...
    if !RunMacro("TCB Run Operation", 26, "Fill Matrices", Opts) then goto badmatrixop
/*
    Opts = null
...
//
//
//	M1 = OpenMatrix(Dir + "\\skims\\TR_PARK_opprmd.mtx", "True")
//        CreateTableFromMatrix(M1, Dir + "\\skims\\TR_PARK_opprmd.asc", "FFA", {{"Complete", "Yes"}})
//    	M1 = null
///////////////////////////////
///////////////////////////////
...
// added by JainM Dec 05
       ...
       rtn_upskim = RunMacro("Update_Drive_Skims_Mtx", "offpeak", "premium", "drive", Args)
...
//	status = RunProgram(pkzip_program + " " + Dir+"\\skims\\TR_SKIM_opprmd.zip " +Dir+"\\skims\\TR_Skim_opprmd.mtx -Add",{{"Minimize", "True"}})
...
//	if status = 0 then DeleteFile(Dir+"\\skims\\TR_Skim_opprmd.mtx")
	...
	badmatrixop:
	msg = msg + {"Reg_OPPrmD - Error in matrix operations"}
	AppendToLogFile(1, "Reg_OPPrmD - Error in matrix operations")
	goto badquit
...
quit:
	...
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit Reg_OPPrmD: " + datentime)
	AppendToLogFile(1, " ")

	return({RegOPPrmDOK, msg})
