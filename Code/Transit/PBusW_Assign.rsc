Macro "PBusW_Assign" (Args)
	...
	AppendToLogFile(1, "Enter PBusW_Assign: " + datentime)
...
// ----------------------------------- STEP 3: Transit Assignment Path Finder  -----------------------------------
     ...
     Opts.Input.[OD Matrix Currency] = {Dir + "\\tranassn\\Transit Assign Walk.mtx", "PbusW", "Rows", "Columns"}
     Opts.Output.[Flow Table] = Dir + "\\tranassn\\PbusW\\TASN_FLW.bin"
     Opts.Output.[Walk Flow Table] = Dir + "\\tranassn\\PbusW\\TASN_WFL.bin"
     Opts.Output.[OnOff Table] = Dir + "\\tranassn\\PbusW\\TASN_ONO.bin"
...
//   ret_value = RunMacro("TCB Run Procedure", 1, "Transit Assignment SP", Opts)  
	...
	badbuildtrannet:
	msg = msg + {"PBusW_Assign - Error return build transit network"}
	AppendToLogFile(1, "PBusW_Assign - Error return build transit network") 
	goto badquit

	badtransettings:
	msg = msg + {"PBusW_Assign - Error return from transit network settings"}
	AppendToLogFile(1, "PBusW_Assign - Error return from transit network settings") 
	goto badquit

	badtranassn:
	msg = msg + {"PBusW_Assign - Error return from transit network skims"}
	AppendToLogFile(1, "PBusW_Assign - Error return from transit network skims")
	goto badquit

	badxpr_stopflags:
	msg = msg + {rtnmsg}
	AppendToLogFile(2, rtnmsg)
	msg = msg + {"PBusW_Assign - Error return from XPR_StopFlags"}
	AppendToLogFile(1, "PBusW_Assign - Error return from XPR_StopFlags")
	goto badquit
	...
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit PBusW_Assign: " + datentime)
	AppendToLogFile(1, " ")

	return({PBusWAssnOK, msg})
