...
//	Replaced field by field fill with sum all fields - field names differ in v.7  JWM Sept. 2016
	...
	nomktseg:
	msg = msg + {"OPPrmDrop_Assign - Zero return from Market_Segment - No assignment"}
	AppendToLogFile(1, "OPPrmDrop_Assign - Zero return from Market_Segment - No assignment") 
	goto badquit
			
	badfill:
	msg = msg + {"OPPrmDrop_Assign - Error return from Fill Dataview - see TC_Report  to determine step"}
	AppendToLogFile(1, "OPPrmDrop_Assign - Error return from Fill Dataview - see TC_Report  to determine step") 
	goto badquit

	badbuildtrannet:
	msg = msg + {"OPPrmDrop_Assign - Error return build transit network"}
	AppendToLogFile(1, "OPPrmDrop_Assign - Error return build transit network") 
	goto badquit

	badtransettings1:
	msg = msg + {"OPPrmDrop_Assign - Error return from transit network settings - Non CBD"}
	AppendToLogFile(1, "OPPrmDrop_Assign - Error return from transit network settings - Non CBD") 
	goto badquit

	badtranassn1:
	msg = msg + {"OPPrmDrop_Assign - Error return from transit network skims - Non CBD"}
	AppendToLogFile(1, "OPPrmDrop_Assign - Error return from transit network skims - Non CBD")
	goto badquit

	badtransettings2:
	msg = msg + {"OPPrmDrop_Assign - Error return from transit network settings - CBD"}
	AppendToLogFile(1, "OPPrmDrop_Assign - Error return from transit network settings - CBD") 
	goto badquit

	badtranassn2:
	msg = msg + {"OPPrmDrop_Assign - Error return from transit network skims - CBD"}
	...
	msg = msg + {rtnmsg}
	AppendToLogFile(2, rtnmsg)
	msg = msg + {"OPPrmDrop_Assign - Error return from XPR_StopFlags"}
