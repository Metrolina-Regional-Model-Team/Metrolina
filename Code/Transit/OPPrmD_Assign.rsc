Macro "OPPrmD_Assign" (Args)

	//Macro to assign OffPeak Premium Drive approach trips
	...
	AppendToLogFile(1, "Enter OPPrmD_Assign: " + datentime)
...
// ----------------------------------- STEP 3: Transit Assignment Path Finder  -----------------------------------
     ...
     Opts.Input.[OD Matrix Currency] = {Dir + "\\tranassn\\Transit Assign Drive.mtx", "OPprmD", "Rows", "Columns"}
     ...
     Opts.Output.[Flow Table] = Dir + "\\tranassn\\OPprmD\\TASN_FLW.bin"
     Opts.Output.[Walk Flow Table] = Dir + "\\tranassn\\OPprmD\\TASN_WFL.bin"
     Opts.Output.[OnOff Table] = Dir + "\\tranassn\\OPprmD\\TASN_ONO.bin"
     Opts.Output.[Stop-stop Matrix].Label = "OPprmD Stop-stop Matrix"
     Opts.Output.[Stop-stop Matrix].[File Name] = Dir + "\\tranassn\\OPprmD\\TASN_S2S.mtx"
	...
	badbuildtrannet:
	msg = msg + {"PPrmD_Assign - Error return build transit network"}
	AppendToLogFile(1, "PPrmD_Assign - Error return build transit network") 
	goto badquit

	badtransettings:
	msg = msg + {"PPrmD_Assign - Error return from transit network settings"}
	AppendToLogFile(1, "PPrmD_Assign - Error return from transit network settings") 
	goto badquit

	badtranassn:
	msg = msg + {"PPrmD_Assign - Error return from transit network skims"}
	...
	msg = msg + {rtnmsg}
	AppendToLogFile(2, rtnmsg)
	msg = msg + {"PPrmD_Assign - Error return from XPR_StopFlags"}
