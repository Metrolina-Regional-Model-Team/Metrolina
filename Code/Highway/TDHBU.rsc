Macro "TDHBU" (Args)
	...
	//Trip Distribution - Home-based University	
	//Friction factors, intrazonal K : Bill Allen Oct, 2013
	//Modified for new UI - Oct, 2015

	//USES HBS1 friction factors (copied into TDhbu.ctl) - TDhbu.ctl renamed TDhbu_old.ctl 
	//  Intra K = 75 (was 23)
	...
	AppendToLogFile(1, "Enter TDhbu: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDhbu.mtx"},
		{"Label", "TDhbu"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\param\\TDhbu.ctl"
	...
	WriteLine(ctlhandle, "TDhbu.set: hbu")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffhbu.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDhbu.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDhbu.txt'")
	...
	WriteLine(ctlhandle, "  skimmax    = 50")
	WriteLine(ctlhandle, "  iter       = 3")
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,50x,f10.3)'")
	WriteLine(ctlhandle, "  ttimefac   = 0.15")
	WriteLine(ctlhandle, "  tlf_file   = t")
	WriteLine(ctlhandle, "  trace      = 0")
	WriteLine(ctlhandle, "  intrak     = 0.1")
	WriteLine(ctlhandle, "/")
	CloseFile(ctlhandle)
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDhbu.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDhbu, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDhbu,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDhbu, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDhbu, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDhbu: " + datentime)
