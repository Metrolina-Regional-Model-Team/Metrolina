Macro "TDATW" (Args)
	...
	//Trip Distribution - Non-home-based- At Work	
	...
	AppendToLogFile(1, "Enter TDATW: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDatw.mtx"},
		{"Label", "TDatw"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDatw.ctl"
	...
	WriteLine(ctlhandle, "TDatw.set: ATW")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffatw.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDatw.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDatw.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,150x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDatw.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDatw, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDatw,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDatw, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDatw, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit

	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDATW: " + datentime)
