Macro "TDHBO3" (Args)
	...
	//Trip Distribution - Home-based Other - income group 3	
	...
	AppendToLogFile(1, "Enter TDhbo3: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDhbo3.mtx"},
		{"Label", "TDhbo3"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	ctlname = METDir + "\\Pgm\\Param\\TDhbo3.ctl"
	...
	WriteLine(ctlhandle, "TDhbo3.set: hbo income 3")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffhbo3.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDhbo3.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\report\\TDhbo3.txt'")
	...
	WriteLine(ctlhandle, "  skimmax    = 60")
	WriteLine(ctlhandle, "  iter       = 3")
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,120x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe "+ METDir + "\\Pgm\\Param\\TDhbo3.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDhbo3, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDhbo3,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDhbo3, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDhbo3, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit

	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDhbo3: " + datentime)
