Macro "ScenarioPlanning_2050MTP" (Args)
...
*/	RunMacro("TCB Init")
	...
	sedata_file = "C:\\19v1w2001SE_ISfix\\Metrolina\\2025\\LandUse\\SE_2025_200710_final.dbf"
	Dir = "C:\\19v1w2001SE_ISfix\\Metrolina\\2025"
	theyear = "2025"
...
// ***  Can change telecommuting percentages here:    ***
  ...
  //files to move over to new Scenario directory
	TCScenarioSubDir = 
		{"\\TD", "\\TripTables"}
	CAVScenarioSubDir = 
		{"\\ModeSplit", "\\Skims", "\\TOD2", "\\TranAssn", "\\HwyAssn\\HOT",
		...
		TDFiles ={{"\\TD",  
			 {"dcATW.bin", "dcATW.dcb", "dcEXT.bin", "dcEXT.dcb",  "dcHBO.bin", "dcHBO.dcb", "dcHBS.bin", "dcHBS.dcb",
			  "dcHBU.bin", "dcHBU.dcb", "dcHBW.bin", "dcHBW.dcb",  "dcSCH.bin", "dcSCH.dcb", 
			  "dcXIN.bin", "dcXIN.dcb", "dcXIW.bin", "dcXIW.dcb",  "dcXIN.bin", "dcXIN.dcb", "dcXIN.bin", "dcXIN.dcb"}} }

		hotassn_dcb = {"Assn_template.dcb"}
...
SetRandomSeed(42837)
  ...
  for tc = 1 to 1 do 	//loop on telecommuting scenarios 
  UpdateProgressBar("Telecommuting Scenario " + i2s(tc), 10) 
  
  	//Create files and move over tables
	DirTCscen = Dir + "\\TC_Scenario" + i2s(tc)
/*  	CreateDirectory(DirTCscen)
  	for i = 1 to TCScenarioSubDir.length do
 	   	CreateDirectory(DirTCscen + TCScenarioSubDir[i])
 	end
		//Copy TD files
		batchname  = DirTCscen + "\\copy_file.bat"
		exist = GetFileInfo(batchname)
		if (exist <> null) then DeleteFile(batchname)
		batchhandle = OpenFile(batchname, "w")

		for i = 1 to TDFiles[1][2].length do
			WriteLine(batchhandle, "copy " + DirOutDC + "\\" + TDFiles[1][2][i] + " " + DirTCscen + "\\TD\\" + TDFiles[1][2][i])
//			WriteLine(batchhandle, "copy " + DirOutDC + "\\" + TDFiles[1][i] + " " + DirTCscen + "\\TD\\" + TDFiles[1][i])
//			WriteLine(batchhandle, "copy " + MRMpath + rundir_files[i] + " " + DirUser + "\\" + rundir_files[i])
		end 
...
//	SetDataVector(hbw_file+"|", "randnumC", randnumC,{{"Sort Order", {{"ID","Ascending"}}}})	
	...
	dcHBO = OpenTable("dcHBO", "FFB", {DirOutDC + "\\dchbo.bin",})
	
	fields_ar = GetFields("dcHBO", null)
...
//Run the rest of model (Trip Accumulator, 
