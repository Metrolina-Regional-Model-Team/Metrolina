Macro "Tour_IS_Location_FB" (Args)

//The Intermediate Stops Location macro just for HBW & ATW (internal tours only) and IX, XIW, and XIN.  
...
// 10/7/20, mk: removed tfac from time-to-last-stop vector(line 398), changed vector to float
  ...
  CreateProgressBar("Starting Tour IS Location Feedback Model", "TRUE")
...
//Start loop on purposes.  HBW & XIN use the peak travel time matrix.  
RandSeed = 132
	for p = 1 to 5 do		

  UpdateProgressBar("IS Location Feedback Iter " + i2s(curiter) + ": " + purp[p], 10) 
...
SetRandomSeed(RandSeed)
	...
	//start PA direction loop, selecting only those tours with intermediate stops
		...
		if ispaset[n] > 0 then do
			tfac = 1
			if p = 3 then tfac = 10	//Validation adjustment for IX (10 for PA direction, 15 for AP direction).
			...
			for s = 1 to ispaset[n] do	//ispaset[n]
				/* Stop location model coefficients from ALOGIT estimation (as of Aug 2015).  Here are the models:
				 ...
				 3 HBW, stop 2             6 HBS/HBO/ATW, stop 1, inc 4    9 all purposes, stops 4-7
					purp = {"HBW", "ATW", "IX", "XIW", "XIN"}
				*/
				m = if (s = 3) then 8 else if (s >3) then 9 else if (s > 1 and (p = 1 or p = 4)) then 3 else if ((p = 1 or p = 4) and incset[n] = 4) then 2 
					else if (p = 1 or p = 4) then 1 else if (s = 2) then 7 else if (incset[n] = 4) then 6 else 5
				if (s > 1) then do
...
//now do AP direction loop, selecting only those tours with intermediate stops
		if isapset[n] > 0 then do
			tfac = 1
			if p = 3 then tfac = 15	//Validation adjustment for IX (10 for PA direction, 15 for AP direction).
...
RandSeed = r2i((RandSeed / 3) + 131)
    ...
    quit:
		on error, notfound default
   		datentime = GetDateandTime()
		AppendToLogFile(1, "Exit IS Location Feedback Iter " + i2s(curiter) + " " + datentime)
    	return({1, msg})
