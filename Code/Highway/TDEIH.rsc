Macro "TDEIH" (Args)
	...
	//Trip Distribution - External - Internal - Heavy Trucks	
	...
	AppendToLogFile(1, "Enter TDeih: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDeih.mtx"},
    		{"Label", "tdeih"},
   			{"File Based", "Yes"},
   			{"Tables", {"Trips"}},
   			{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDeih.ctl"
	...
	WriteLine(ctlhandle, "TDeih.set: E/I HTK")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffeih.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDeih.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDeih.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,290x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDeih.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDeih, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDeih,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDeih, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDeih, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit

	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDeih: " + datentime)
