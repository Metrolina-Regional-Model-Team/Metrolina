Macro "TDHBW3" (Args)
	...
	//Trip Distribution - Home-based Work - income group 3	
	...
	AppendToLogFile(1, "Enter TDhbw3: "  + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDhbw3.mtx"},
		{"Label", "TDhbw3"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDhbw3.ctl"
	...
	WriteLine(ctlhandle, "TDhbw3.set: HBW income 3")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffhbw3.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDhbw3.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDhbw3.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,20x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDhbw3.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDhbw3, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDhbw3,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDhbw3, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDhbw3, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDhbw3: " + datentime)
