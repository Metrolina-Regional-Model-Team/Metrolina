Macro "TDNWK" (Args)
	...
	//Trip Distribution - Non-home-based non-work related trips	
	...
	AppendToLogFile(1, "Enter TDnwk: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDnwk.mtx"},
		{"Label", "TDnwk"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	ctlname = METDir + "\\Pgm\\Param\\TDnwk.ctl"
	...
	WriteLine(ctlhandle, "TDnwk.set: nwk")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '"+METDir + "\\Pgm\\frictionfactors\\ffnwk.prn'")
	WriteLine(ctlhandle, "  prods    = '"+Dir+"\\tg\\productions.asc'")
	WriteLine(ctlhandle, "  attrs    = '"+Dir+"\\tg\\attractions.asc'")
	WriteLine(ctlhandle, "  htime    = '"+ Dir + "\\Skims\\TThwy_free.mtx',")
	WriteLine(ctlhandle, "  ttime    = '"+ Dir + "\\Skims\\TTTran_free.mtx',")
	WriteLine(ctlhandle, "  trips    = '"+ Dir + "\\TD\\TDnwk.mtx'")
	WriteLine(ctlhandle, "  listing  = '"+Dir+"\\report\\TDnwk.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,160x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDnwk.ctl")
	CloseFile(ctlhandle)
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDnwk: " + datentime)
