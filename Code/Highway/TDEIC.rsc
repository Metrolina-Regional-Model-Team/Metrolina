Macro "TDEIC" (Args)
	...
	//Trip Distribution - External - Internal Commercial Vehicles	
	...
	AppendToLogFile(1, "Enter TDeic: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDeic.mtx"},
		{"Label", "TDeic"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDeic.ctl"
	...
	WriteLine(ctlhandle, "TDeic.set: E/I COM")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffeic.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDeic.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDeic.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,270x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDeic.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDeic, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDeic,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDeic, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDeic, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit

	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDeic: " + datentime)
