Macro "TDSCH" (Args)
	...
	//Trip Distribution - Home-based school	
	...
	AppendToLogFile(1, "Enter TDsch: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDsch.mtx"},
		{"Label", "TDsch"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\param\\TDsch.ctl"
	...
	WriteLine(ctlhandle, "TDsch.set: sch")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '"+METDir + "\\Pgm\\frictionfactors\\ffsch.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '"+ Dir + "\\TD\\TDsch.mtx'")
	WriteLine(ctlhandle, "  listing  = '"+Dir+"\\report\\TDsch.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,40x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDsch.ctl")
	CloseFile(ctlhandle)
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDsch: " + datentime)
