Macro "TDJTW" (Args)
	...
	//Trip Distribution - Non-home-based Journey to Work	
	...
	AppendToLogFile(1, "Enter TDjtw: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDjtw.mtx"},
		{"Label", "TDjtw"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDjtw.ctl"
	...
	WriteLine(ctlhandle, "TDjtw4.set: jtw")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffjtw.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDjtw.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDjtw.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,140x,f10.3)'")
	WriteLine(ctlhandle, "  ttimefac   = 0.03")
	WriteLine(ctlhandle, "  tlf_file   = t")
	WriteLine(ctlhandle, "  trace      = 1")
	WriteLine(ctlhandle, "  intrak     = 2.0")
	WriteLine(ctlhandle, "/")
	CloseFile(ctlhandle)
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDjtw.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDjtw, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDjtw,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDjtw, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDjtw, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDjtw: " + datentime)
