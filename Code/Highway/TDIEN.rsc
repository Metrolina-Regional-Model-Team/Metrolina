Macro "TDIEN" (Args)
	...
	//Trip Distribution - Internal - external - non-work trips	
	...
	AppendToLogFile(1, "Enter TDien: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDien.mtx"},
		{"Label", "TDien"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDien.ctl"
	...
	WriteLine(ctlhandle, "TDien.set: I/E Non-Work Auto")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffien.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDien.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDien.txt'")	
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,210x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDien.ctl")
	CloseFile(ctlhandle)
	...
	fortbadrun:
	Throw("TDien, Fatal error in fortran program tdmet_mtx (trip distribution)")
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDien: " + datentime)
