Macro "TDEIW" (Args)
	...
	//Trip Distribution - External - Internal - Work trips	
	...
	AppendToLogFile(1, "Enter TDeiw: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDeiw.mtx"},
		{"Label", "TDeiw"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDeiw.ctl"
	...
	WriteLine(ctlhandle, "TDeiw.set: E/I Work Auto")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffeiw.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDeiw.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDeiw.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,250x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe "+METDir + "\\pgm\\param\\TDeiw.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDeiw, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDeiw,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDeiw, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDeiw, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDeiw: " + datentime)
