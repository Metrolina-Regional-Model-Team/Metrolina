Macro "TDEIN" (Args)
	...
	//Trip Distribution - External - Internal - Non-work	
	...
	AppendToLogFile(1, "Enter TDein: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDein.mtx"},
		{"Label", "TDein"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\pgm\\Param\\TDein.ctl"
	...
	WriteLine(ctlhandle, "TDein.set: E/I Non-Work Auto")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffein.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDein.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDein.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,260x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe "+ METDir + "\\Pgm\\Param\\TDein.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDein, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDein,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDein, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDein, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDein: " + datentime)
