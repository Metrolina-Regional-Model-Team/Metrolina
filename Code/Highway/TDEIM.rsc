Macro "TDEIM" (Args)
	...
	//Trip Distribution - External - internal - Medium trucks	
	...
	AppendToLogFile(1, "Enter TDEIM: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDeim.mtx"},
		{"Label", "TDeim"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDeim.ctl"
	...
	WriteLine(ctlhandle, "TDeim.set: E/I MTK")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffeim.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDeim.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDeim.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,280x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDeim.ctl")
	CloseFile(ctlhandle)
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDeim " + datentime)
