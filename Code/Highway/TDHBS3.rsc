Macro "TDHBS3" (Args)
	...
	//Trip Distribution - Home-based Shopping - income group 3	
	...
	AppendToLogFile(1, "Enter TDhbs3: " + datentime)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDhbs3.mtx"},
		{"Label", "TDhbs3"},
		{"File Based", "Yes"},
		{"Tables", {"Trips"}},
		{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDhbs3.ctl"
	...
	WriteLine(ctlhandle, "TDhbs3.set: hbs income 3")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffhbs3.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDhbs3.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDhbs3.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,80x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDhbs3.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDhbs3, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDhbs3,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDhbs3, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDhbs3, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit
	...
	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDhbs3: " + datentime)
