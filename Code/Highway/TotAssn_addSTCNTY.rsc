...
//	goto skiparound
	...
	cntflag		= CreateExpression(netview, "CNTFLAG", "if CALIB15 > 0 then 1 else 0",)
...
// drop transit links
	...
	netfields = {"ID", "LENGTH", "DIR", "FUNCL", "FEDFUNC_AQ", "CO_FEDFUNC", "Strname", "A_CrossStr", "B_CrossStr", "AREATP", "CALIB15", "MTK15", "HTK15", "Scrln", 
...
//Daily fields
	...
	cntmcsq		= CreateExpression(join4, "CNTMCSQ", "if CALIB15 > 0 then pow(TOT_VOL - CALIB15,2) else 0",)
//	commcsq		= CreateExpression(join4, "COMMCSQ", "if COM08 > 0 then pow(TOT_COM - COM08,2) else 0",)
	mtkmcsq		= CreateExpression(join4, "MTKMCSQ", "if MTK15 > 0 then pow(TOT_MTK - MTK15,2) else 0",) 
	htkmcsq		= CreateExpression(join4, "HTKMCSQ", "if HTK15 > 0 then pow(TOT_HTK - HTK15,2) else 0",)

	if typeassn = "base" then do
		OutFields = 
		{"ID", "LENGTH", "DIR", "FUNCL", "FEDFUNC_AQ", "CO_FEDFUNC", "Strname", "A_CrossStr", "B_CrossStr", 
	  	 "AREATP", "CALIB15", "MTK15", "HTK15", "Scrln", "COUNTY", "STCNTY", "Cap1hrAB", "Cap1hrBA", 
...
/*	else if typeassn = "HOT2+" then do
		OutFields = 
		{"ID", "LENGTH", "DIR", "FUNCL", "FEDFUNC_AQ", "CO_FEDFUNC", "Strname", "A_CrossStr", "B_CrossStr", 
	  	 "AREATP", "CALIB15", "MTK15", "HTK15", "Scrln", "COUNTY", "Cap1hrAB", "Cap1hrBA", 
	 	 "TTfreeAB", "TTfreeBA", "TTPkAssnAB", "TTPkAssnBA", "lanesAB", "lanesBA",
	 	 "Fun2", "CntyAF", "VMTLen", 
	 	 "VOL_POST", "Tot_Vol",   
