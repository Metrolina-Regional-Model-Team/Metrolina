Macro "TDCOM" (Args)
	...
	CopyMatrixStructure({templatecore}, {{"File Name", Dir + "\\TD\\TDcom.mtx"},
    		{"Label", "TDcom"},
   			{"File Based", "Yes"},
   			{"Tables", {"Trips"}},
   			{"Operation", "Union"}})
	...
	//TDMET control file
	ctlname = METDir + "\\Pgm\\Param\\TDcom.ctl"
	...
	WriteLine(ctlhandle, "TDcom.set: com")
	WriteLine(ctlhandle, " ")
	...
	WriteLine(ctlhandle, " ")
	WriteLine(ctlhandle, "&files")
	WriteLine(ctlhandle, "  ffactors = '" + METDir + "\\Pgm\\Frictionfactors\\ffcom.prn'")
	...
	WriteLine(ctlhandle, "  trips    = '" + Dir + "\\TD\\TDcom.mtx'")
	WriteLine(ctlhandle, "  listing  = '" + Dir + "\\Report\\TDcom.txt'")
	...
	WriteLine(ctlhandle, "  tformat    = '(i10,i10,170x,f10.3)'")
	...
	if (exist <> null) then DeleteFile(ctlname)
	ctlhandle = OpenFile(ctlname, "w")
	WriteLine(ctlhandle, METDir + "\\Pgm\\tdmet_mtx.exe " + METDir + "\\Pgm\\Param\\TDcom.ctl")
	CloseFile(ctlhandle)
	...
	fortdidnotrun:
	msg = msg + {"TDcom, ERROR - Fortran job tdmet_mtx (trip distribution) did not run!"}
	AppendToLogFile(1, "TDcom,  ERROR - Fortran tdmet_mtx (trip distribution) did not run!")
	TripDistOK = 0
	goto quit

	fortbadrun:
	msg = msg + {"TDcom, Fatal error in fortran program tdmet_mtx (trip distribution)"}
	AppendToLogFile(1, "TDcom, Fatal error in fortran program tdmet_mtx (trip distribution)")
	TripDistOK = 0
	goto quit

	quit:
	datentime = GetDateandTime()
	AppendToLogFile(1, "Exit TDcom " + datentime)
