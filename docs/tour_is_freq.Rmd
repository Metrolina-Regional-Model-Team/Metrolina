...
est_tbl <- tours %>%
  ...
  mutate(
    int_stop_fwd_cap = ifelse(int_stop_fwd > 2, 2, int_stop_fwd),
    int_stop_ret_cap = ifelse(int_stop_ret > 2, 2, int_stop_ret)
  ) %>%
...
fwd_pcts <- est_tbl %>%
  filter((tour_type == "shop" & int_stop_fwd >= 3) | (tour_type != "shop" & int_stop_fwd >= 2)) %>%
  mutate(int_stop_fwd = ifelse(int_stop_fwd > 5, 5, int_stop_fwd)) %>%
...
ret_pcts <- est_tbl %>%
  filter((tour_type == "shop" & int_stop_ret >= 3) | (tour_type != "shop" & int_stop_ret >= 2)) %>%
  mutate(int_stop_ret = ifelse(int_stop_ret > 5, 5, int_stop_ret)) %>%
