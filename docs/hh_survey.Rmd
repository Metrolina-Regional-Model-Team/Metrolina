...
tours_by_hh <- tour_file %>%
  ...
  group_by(hh_id) %>%
  summarize(across(escort:WorkersThatDay, ~ sum(.x))) %>%
  rename_with(~ paste0(.x, "_tours"), escort:school)
  

hh_add_tours <- hh_add_strct %>%
  left_join(tours_by_hh, by = "hh_id") %>%
  mutate(across(escort_tours:WorkersThatDay, ~ifelse(is.na(.x), 0, .x))) %>%
...
per_add_tour_counts <- per_calc_ptypes %>%
  ...
  mutate(
    across(escort_tours:school_tours, ~ifelse(is.na(.), 0, .))
  )
```
