...
id_sub_tours <- mark_anchor %>%
  mutate(
    # need a location more accurate than TAZs, but round off 5th decimal so
    # that we get a roughly 36' square grid points.
    d_loc = paste(round(d_lon, 4), round(d_lat, 4), sep = "-"),
    work_anchor = ifelse(anchor == 1 & d_purpose_linked == "W", paste0("w-", d_loc), NA),
    work_anchor = first(work_anchor[!is.na(work_anchor)]),
    check = ifelse(d_purpose_linked == "W", paste0("w-", d_loc), ""),
    check = ifelse(check == work_anchor, 1, 0),
    # # a handful of tours contain only trips ending at the same place for work.
    # # don't consider these subtours.
    sum = sum(check, na.rm = TRUE),
    count = n(),
    check = ifelse(sum == count, 0, check),
    cume = cumsum(check),
    subtour_id = cume - check,
    max = max(subtour_id),
    subtour_id = ifelse(subtour_id == max, NA, subtour_id),
    subtour_id = ifelse(subtour_id <= 0, NA, subtour_id),
    subtour_id = ifelse(!is.na(subtour_id), paste0(tour_id, "_", subtour_id), NA)
  ) %>%
  select(-c(d_loc:cume, max))
...
hh_add_tours <- hh_add_strct %>%
  ...
  left_join(subtours_by_hh, by = "hh_id") %>%
