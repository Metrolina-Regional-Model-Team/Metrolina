...
```{r}
model_tours <- read_csv("data/input/tour_freq/model_results.csv")
...
model_shares <- model_tours %>%
  ...
  summarize(n = n()) %>%
  mutate(
    model_pct = round(n / sum(n, na.rm = TRUE) * 100, 2),
    tour_type = case_when(
...
cal_tbl <- est_tbl %>%
  ...
  left_join(weights, by = "hh_id") %>%
  group_by(tour_type, count) %>%
  summarize(expanded_tours = sum(hh_weight, na.rm = TRUE)) %>%
  filter(tour_type != "sub_tours") %>%
  mutate(survey_pct = round(expanded_tours / sum(expanded_tours) * 100, 1)) %>%
  left_join(
    model_shares %>% select(-n), by = c("tour_type", "count")
  ) %>%
  select(-expanded_tours) %>%
